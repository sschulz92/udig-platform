...
public class WMSCTileUtils {
    ...
    private static class PreloadTilesClass implements IRunnableWithProgress {
         ...
         */
        private BlockingQueue<Tile> tilesCompleted_queue = new PriorityBlockingQueue<>();
        ...
        public void run(IProgressMonitor monitor)
            ...
            String taskname = MessageFormat.format(Messages.WMSCTileUtils_preloadtask,
                    tileset.getLayers());
            ...
            double percentPerResolution = resolutions.length / 100.0;
            tileRangeTiles = new HashMap<>();
            ...
            try {
                for (double resolution : resolutions) {
                    ...
                    if (select != null) {
                        boundsList = tileset.getBoundsListForZoom(JTS.toEnvelope(select),
                                resolution);
                        totalTilesForZoom = tileset.getTileCount(JTS.toEnvelope(select),
                                resolution);
                    } else {
                    ...
                    while (boundsIter.hasNext()) {
                        ...
                        Map<String, Tile> allTilesInZoom = tileset.getTilesFromZoom(env,
                                resolution);
                        Map<String, Tile> tilesInZoom = new HashMap<>();
        ...
        private void doRequestAndResetVars(final TileSet tileset) {
            ...
            Map<String, Tile> loadedTiles = new HashMap<>();
            ...
            while (!tileRangeTiles.isEmpty()) {
                Tile tile = null;
                try {
                    tile = tilesCompleted_queue.take(); // blocks until a tile is done
    ...
    public static TileSet toTileSet(IGeoResource resource, AbstractOpenWebService<?, ?> server,
        ...
        try {
            ...
            if (source == null || "".equals(source)) { //$NON-NLS-1$
                WmsPlugin.log("GetCapabilities SERVICE is required", //$NON-NLS-1$
                        new ServiceNotFoundException());
                return null;
            }
            if (version == null || "".equals(version)) { //$NON-NLS-1$
                WmsPlugin.log("GetCapabilities VERSION is required", //$NON-NLS-1$
                        new ServiceNotFoundException());
                return null;
            }
            ...
            if (bounds == null) {
                WmsPlugin.log("Bounds required for TileSet definition", //$NON-NLS-1$
                        new NullPointerException("Bounds required for tileset definitio")); //$NON-NLS-1$
                return null;
            }
            ...
            Integer width = Integer
                    .parseInt((String) properties.get(PreferenceConstants.P_TILESET_WIDTH));
            Integer height = Integer
                    .parseInt((String) properties.get(PreferenceConstants.P_TILESET_HEIGHT));
            ...
            if ("".equals(resolutions)) { //$NON-NLS-1$
                WmsPlugin.log("Resolutions are required for TileSet generation", //$NON-NLS-1$
                        new ServiceNotFoundException());
                return null;
            }
     ...
     */
    public static String workoutResolutions(String rawScales, ReferencedEnvelope bounds,
            int tileWidth) {
