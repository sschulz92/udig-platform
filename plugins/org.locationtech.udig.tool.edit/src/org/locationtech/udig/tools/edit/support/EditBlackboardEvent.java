...
public class EditBlackboardEvent {
    ...
    public static enum EventType {
         ...
         * <ul>
         * <li>The source is a Collection of {@link PrimitiveShape} that were affected by the change</li>
         ...
         * <li>The OldValue is the Collection of {@link EditGeom} objects that were in the blackboard
         ...
         * <li>The NewValue is a Map<Point, List<Point>> where the key is the original point and the value
         * is the new location(s) of the point after translation on a zoom in a single point can
         * become more than one point.</li>
         * </ul>
         */
        TRANFORMATION,
        /**
    ...
    private void assertLegalValues( Object source2, EventType type2, Object before2, Object after2 ) {
        ...
        case ADD_GEOMS:
            if (!(source2 instanceof EditBlackboard))
                throw new IllegalArgumentException("The source of a "+type2+" event must be a " + //$NON-NLS-1$ //$NON-NLS-2$
                        "EditBlackboard was "+source2.getClass().getName()); //$NON-NLS-1$
            if (before2 != null)
                throw new IllegalArgumentException("The before value of a "+type2+" event be null was "+before2.getClass().getName()); //$NON-NLS-1$ //$NON-NLS-2$
            if (!(after2 instanceof Collection))
                throw new IllegalArgumentException("The after value of a "+type2+" event must be " + //$NON-NLS-1$ //$NON-NLS-2$
                        "be a Collection was "+after2.getClass().getName()); //$NON-NLS-1$
            if (((Collection) after2).size() < 1)
                throw new IllegalArgumentException("The after value of a "+type2+" event must be " + //$NON-NLS-1$ //$NON-NLS-2$
                        "be larger than 0" ); //$NON-NLS-1$
            for( Iterator<Object> iter = ((Collection<Object>) after2).iterator(); iter.hasNext(); ) {
        ...
        case ADD_POINT:
            ...
            if (before2 != null)
                throw new IllegalArgumentException(
                        "The before value of a "+type2+" event be null was " + before.getClass().getName()); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        case MOVE_POINT:
            ...
            if (((Set) source2).size() < 1)
                throw new IllegalArgumentException("The source value of a "+type2+" event must be " + //$NON-NLS-1$ //$NON-NLS-2$
                        "be larger than 0" ); //$NON-NLS-1$
            ...
            if (!(before2 instanceof Point) )
                throw new IllegalArgumentException(
                        "The before value of a "+type2+" event be a Point was " + before.getClass().getName()); //$NON-NLS-1$ //$NON-NLS-2$
            if (!(after2 instanceof Point))
                throw new IllegalArgumentException("The after value of a "+type2+" event must be " + //$NON-NLS-1$ //$NON-NLS-2$
                        "be a Point was " + after2.getClass().getName()); //$NON-NLS-1$
            break;
        case REMOVE_GEOMS:
            ...
            if (!(before2 instanceof Collection) )
                throw new IllegalArgumentException(
                        "The before value of a "+type2+" event be a EditGeom was " + before.getClass().getName()); //$NON-NLS-1$ //$NON-NLS-2$
            if ( after2 != null)
                throw new IllegalArgumentException("The after value of a "+type2+" event must be null" ); //$NON-NLS-1$ //$NON-NLS-2$
            break;
        ...
        case SET_GEOMS:
            if (!(source2 instanceof EditBlackboard))
                throw new IllegalArgumentException("The source of a "+type2+" event must be a " + //$NON-NLS-1$ //$NON-NLS-2$
                        "EditBlackboard was "+source2.getClass().getName()); //$NON-NLS-1$
            if (!(before2 instanceof Collection))
                throw new IllegalArgumentException("The before value of a "+type2+" event must be " + //$NON-NLS-1$ //$NON-NLS-2$
                        "be a Collection was "+before2.getClass().getName()); //$NON-NLS-1$
            if (((Collection) before2).size() < 1)
                throw new IllegalArgumentException("The before value of a "+type2+" event must be " + //$NON-NLS-1$ //$NON-NLS-2$
                        "be larger than 0" ); //$NON-NLS-1$
            for( Iterator<Object> iter = ((Collection<Object>) before2).iterator(); iter.hasNext(); ) {
                Object obj = iter.next();
                if (!(obj instanceof EditGeom))
                    throw new IllegalArgumentException(
                            "The before value of a "+type2+" event must be " + //$NON-NLS-1$ //$NON-NLS-2$
                                    "be a Collection of EditGeoms found a " + obj.getClass().getName()); //$NON-NLS-1$
            }
        ...
        case TRANFORMATION:
            ...
            case ADD_POINT_TO_MANY:
                ...
                for( Iterator<Object> iter = ((Collection<Object>) source2).iterator(); iter.hasNext(); ) {
                    Object obj = iter.next();
                    if (!(obj instanceof PrimitiveShape))
                        throw new IllegalArgumentException(
                                "The after value of a "+type2+" event must be " + //$NON-NLS-1$ //$NON-NLS-2$
