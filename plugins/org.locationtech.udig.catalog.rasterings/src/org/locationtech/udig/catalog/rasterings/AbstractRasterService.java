...
public abstract class AbstractRasterService extends IService {
    ...
    public <T> boolean canResolve(Class<T> adaptee) {
        return adaptee != null && (adaptee.isAssignableFrom(AbstractGridCoverage2DReader.class)
    ...
    public synchronized GridCoverage2DReader getReader(IProgressMonitor monitor) {
        if (this.reader == null) {
            try {
                AbstractGridFormat frmt = getFormat();
                ID id = getID();
                if (id.isFile()) {
                    File file = id.toFile();
                    if (file != null) {
                        ...
                        this.reader = frmt.getReader(file);
                        return this.reader;
                    } else {
                        throw new FileNotFoundException(id.toFile().toString());
                    }
                }
                this.reader = frmt.getReader(id.toURL());
            } catch (Exception ex) {
                this.message = ex;
            }
        }
