...
public class ImportFrom implements IOp {
     ...
     * @return <code>true</code> if non <code>null</code>, subclass can override with additional tests
     */
    ...
    protected class PromptAndImport implements Runnable {
         ...
         * @param monitor
         */
        public PromptAndImport( Display display, Object target, IProgressMonitor monitor ){
    ...
    public File promptFile( Display display, Object target ) {
        ...
        if( !file.exists() ){
            MessageDialog.openError(display.getActiveShell(), prompt,
                file.getAbsolutePath()+" does not exist." ); //$NON-NLS-1$
            return null;
       }
       return file;
    }
