...
public class BasicWMSRenderer2 extends RendererImpl implements IMultiLayerRenderer {
    ...
    public synchronized void render(Graphics2D destination, ReferencedEnvelope bounds,
        ...
        try {
            ...
            if (capabilities.getRequest().getGetMap().getFormats()
            ...
            for (int i = layers.size() - 1; i >= 0; i--) {
                ...
                if (currScale >= minScale && currScale <= maxScale) {
                    ...
                    StyleImpl wmsStyle = (StyleImpl) ilayer.getStyleBlackboard()
                            .get(WMSStyleContent.WMSSTYLE);
                    if (wmsStyle != null) {
            ...
            if (requestBBox.getCoordinateReferenceSystem().equals(viewportCRS)) {
                backprojectedBBox = requestBBox;
            } else {
                backprojectedBBox = requestBBox.transform(viewportCRS, true);
            }
            ...
            Dimension maxDimensions = new Dimension(wmsService.getMaxWidth(),
                    wmsService.getMaxHeight());
            ...
            Dimension imageDimensions = calculateImageDimensions(getContext().getImageSize(),
            ...
            String srs = identifiers.isEmpty() ? EPSG_4326
                    : identifiers.iterator().next().toString();
    ...
    private void setFilter(WebMapServer wms, GetMapRequest request) {
        ...
        Map<ILayer, Filter> filters = new HashMap<>();
        List<ILayer> layers = getContext().getLayers();
        for (ILayer layer : layers) {
            Object layerFilter = layer.getStyleBlackboard()
                    .get(ProjectBlackboardConstants.LAYER__DATA_QUERY);
            Filter filter;
    ...
    private void renderGridCoverage(Graphics2D graphics, Envelope bounds, Dimension dimension,
        ...
        Point upperLeft = getContext()
                .worldToPixel(new Coordinate(envelope.getMinX(), envelope.getMinY()));
        Point bottomRight = getContext()
                .worldToPixel(new Coordinate(envelope.getMaxX(), envelope.getMaxY()));
        ...
        AffineTransform worldToScreen = RendererUtilities.worldToScreenTransform(envelope,
                screenSize, destinationCRS);
        GridCoverageRenderer paint = new GridCoverageRenderer(destinationCRS, envelope, screenSize,
                worldToScreen);

        RasterSymbolizer symbolizer = CommonFactoryFinder.getStyleFactory(null)
    ...
    private GridCoverage2D convertImageToGridCoverage(ReferencedEnvelope requestBBox,
        ...
        GridCoverage2D gc = factory.create("GridCoverage", image, gtEnv); //$NON-NLS-1$
        return gc;
    }
     ...
     */
    public static Dimension calculateImageDimensions(Dimension displaySize, Dimension maxDimensions,
            Envelope viewport, Envelope request) throws RenderException {
    ...
    private static ReferencedEnvelope swapAxis(ReferencedEnvelope envelope) {
        ...
        ReferencedEnvelope swap = new ReferencedEnvelope(min1, max1, min0, max0,
                envelope.getCoordinateReferenceSystem());

        return swap;
    }

    public static ReferencedEnvelope getLayersBoundingBox(CoordinateReferenceSystem crs,
            List<Layer> layers, String version) {
     ...
     * last request on the layer blackboard for this intra module communication.
     *
     * @return SRS code
     */
    ...
    private static void dontFind(final IMap map, final String DONT_FIND) {
        map.getBlackboard().put(EPSG_CODE, DONT_FIND);
    }
    ...
    private List<ILayer> getLayers() {
        ...
        IRenderContext[] contexts = context1.getContexts()
                .toArray(new IRenderContext[context1.getContexts().size()]);
