...
/**
 * Opens a map when dropped on an MapPart
 *
 * @author jones
 * @since 1.1.0
 */
public class DropMap extends IDropAction {
    ...
    public boolean accept() {
        ...
        if (getDestination() instanceof ILayer) {
            ...
            if (control != LayersView.getViewer().getControl()
                    && !(control instanceof ViewportPane))
                return false;
        }
        ...
        if (getData() instanceof EObject && !(getDestination() instanceof ILayer)) {
            ...
            if (getData() instanceof ILayer && getDestination() instanceof LayersView
    ...
    public void perform(IProgressMonitor monitor) {
        ...
        if (url != null) {
            ...
            try {
                ...
                File file = new File(fileString);
                if (!file.exists() || !fileString.endsWith(".umap")) { //$NON-NLS-1$
                    ProjectUIPlugin.log("Some how accept() accepted: " + getData(), //$NON-NLS-1$
                            new Exception());
                    return;
                }
            } catch (Exception e) {
