...
public class ToolProxy extends ModalItem implements ModalTool, ActionTool {
    ...
    public ToolProxy(IExtension extension, IConfigurationElement definition,
        ...
        OpFilter parseEnablement = EnablementUtil.parseEnablement(
                extension.getNamespaceIdentifier() + "." + definition.getName(), //$NON-NLS-1$
                definition.getChildren("enablement")); //$NON-NLS-1$ ;
    ...
    private List<OperationCategory> parseOperationCategories(IConfigurationElement toolElement) {
        ...
        Map<String, OperationCategory> categories = UiPlugin.getDefault().getOperationMenuFactory()
                .getCategories();
        ...
        for (IConfigurationElement element : children) {
            ...
            if (opCategoryID == null || opCategoryID.length() == 0) {
                ProjectUIPlugin
                        .log("Warning: CategoryID attribute of operationCategory element in tool '" //$NON-NLS-1$
                                + id + "' is empty.", null); //$NON-NLS-1$
                continue;
            }
            ...
            if (opCategory == null) {
                ProjectUIPlugin
                        .log("Warning: CategoryID attribute of operationCategory element in tool '" //$NON-NLS-1$
                                + id + "' cannot be found. Does it actually exist?", null); //$NON-NLS-1$
                continue;
            }
    ...
    public void setContext(IToolContext toolContext) {
        ...
        Tool currentTool = getTool();
        if (currentTool != null) {
            if (type == BACKGROUND) {
                currentTool.setContext(toolContext);

            } else if (type == MODAL && toolManager.getActiveToolProxy() == this) {
                ModalTool modalTool = getModalTool(); // should also be not null (cast of the
                                                      // current tool)
                modalTool.setContext(toolContext);
                String currentCursorID = modalTool.getCursorID();
                toolContext.getViewportPane()
                        .setCursor(ApplicationGIS.getToolManager().findToolCursor(currentCursorID));
    ...
    /**
     * Called when the user clicks on the button or otherwise "Activates" the tool represented here.
     */
    @Override
    public void run() {
        if (isModeless()) {
    ...
    protected void runModeless() {
        ...
        PlatformGIS.run(new ISafeRunnable() {
            ...
            public void handleException(Throwable exception) {
                ProjectUIPlugin.log("Error occured while executing tool: " + getId(), exception); //$NON-NLS-1$
            }
        });
    }
    ...
    public void setActive(boolean active) {
        ...
        if (getTool() instanceof ModalTool) {
            if (isActive() != active) {
                ...
                if (active) {
                    // allow tool manager to update the activeTool cursor etc.. --> calls
                    // setActive()
                    toolManager.setActiveModalToolProxy(this);
                    // String currentCursorID = modalTool.getCursorID();
                    // toolContext.getViewportPane().setCursor(toolManager.findToolCursor(currentCursorID));
                } else {
                    modalTool.setActive(active); // only called when deactivated, because it gets
                                                 // also called by
                                                 // toolManager.setActiveModalToolProxy
                }
            }
        }
    }
    ...
    public String getCursorID() {
        throw new UnsupportedOperationException(
                "Call ToolProxy.getTool().getCursorID() method instead"); //$NON-NLS-1$
    }
    ...
    public void setCursorID(String id) {
        throw new UnsupportedOperationException(
                "Call ToolProxy.getTool().setCursorID(String) method instead"); //$NON-NLS-1$
    }
    ...
    public Object getProperty(String key) {
        throw new UnsupportedOperationException(
                "Call ToolProxy.getTool().getProperty(String) method instead"); //$NON-NLS-1$
    }
    ...
    public void setProperty(String key, Object value) {
        throw new UnsupportedOperationException(
                "Call ToolProxy.getTool().setProperty(String, String) method instead"); //$NON-NLS-1$
    ...
    public void addListener(ToolLifecycleListener listener) {
        throw new UnsupportedOperationException(
                "Call ToolProxy.getTool().addListener(ToolLifecycleListener) method instead"); //$NON-NLS-1$
    ...
    public void removeListener(ToolLifecycleListener listener) {
        throw new UnsupportedOperationException(
                "Call ToolProxy.getTool().removeListener(ToolLifecycleListener) method instead"); //$NON-NLS-1$
    }
    ...
    public void setSelectionProvider(IMapEditorSelectionProvider selectionProvider) {
        throw new UnsupportedOperationException(
                "Call ToolProxy.getTool().setSelectionProvider(IMapEditorSelectionProvider) method instead"); //$NON-NLS-1$
    ...
    public boolean isTogglingButtonEnabled() {
        Tool currentTool = getTool();
        return currentTool != null && currentTool.isTogglingButtonEnabled();
    }
