 ...
 * <p>
 * A compatible feature type are feature types that have the same attributes types but maybe in a
 * different order and the second (the type being adapted to) may have fewer attribute types.
 * </p>
 *
 * @author Jesse
 */
class FeatureWrapper extends DecoratingFeature implements SimpleFeature {
     ...
     * @param geometries the geometries to use instead of the feature's geometries this allows the
     *        geometries to be transformed
     * @param geomAttNames the attribute names of the geometries so we know how to put them in the
     *        attribute array.
     *
     * @throws IllegalAttributeException Thrown if the default geometry does not exist
     */
    public FeatureWrapper(final SimpleFeature wrapped, final SimpleFeatureType featureType,
            Geometry[] geometries, String[] geomAttNames) throws IllegalArgumentException {
        super(wrapped);
    ...
    private Geometry findTransformedGeometry(String defaultGeomName)
    ...
    public List<Object> getAttributes() {
        List<Object> attributes = new ArrayList<>(delegate.getAttributes());
        for (int i = 0; i < attributes.size(); i++) {
            ...
            if (attributeType instanceof GeometryDescriptor) {
                attributes.set(index,
                        findTransformedGeometry(attributeType.getName().getLocalPart()));
            } else {
